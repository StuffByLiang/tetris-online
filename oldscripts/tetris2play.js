...
var game = {
    randomizeBag : function() {
        ...
        bag.shuffle(this.bag[this.bag.length-1]); //shuffle bag
        ...
        for(var y=0; y <= 21; y++) {// every row
            for(var x=0; x<= 9; x++) { //go through every block in the row
                if(this.boardPosition[x][y] !== 0){
                    ...
                    draw.makeBlock(1 + x * 24, 1 + (y - enemyBoard.playerGarbage) * 24, 0, 0, color, tetrisBoard.canvas);
                }
            }
        }
        //draw garbage
        for(var y=21; y > 21 - enemyBoard.playerGarbage; y--) {// every row
            for(var x=0; x<= 9; x++) { //go through every block in the row
                draw.makeBlock(1 + x * 24, 1 + (y) * 24, 0, 0, "#d3d3d3", tetrisBoard.canvas);
            }
        }
    ...
    spawnPiece : function() {
        ...
        //check k-o
        var check = false;
        for(var y = 0 + enemyBoard.playerGarbage; y <= 1 + enemyBoard.playerGarbage; y++){
            for(var x = 3; x <=6; x++){
        ...
        if(check){
            ...
            enemyBoard.enemyKo++;
            document.getElementById("enemy").innerHTML = "KO's: " + enemyBoard.enemyKo;
        }

        this.piece = new this.pieceObject(3, -1 + enemyBoard.playerGarbage, player.currentPieceName, color, rotations); //create piece
        ...
        if(key.pressed[key.down] > 0) {
            ...
            if(!game.piece.checkCollision(1)){
                ...
                game.piece.interval = setInterval(game.piece.gravityInterval, 30);
            }
        }
    ...
    pieceObject : function(x, y, pieceName, color, rotations) {
        ...
        this.update = function () {
            ...
            for(var i = 0; i <= 3; i++) {
                ...
                draw.makeBlock(1 + (game.piece.x) * 24, 1 + (game.piece.y - enemyBoard.playerGarbage) * 24, xx, yy, color, tetrisBoard.canvas);
            }
    ...
    ghostObject : function(color) {
        ...
        this.update = function() {
            ...
            for(var i = 0; i <= 3; i++) {
                ...
                draw.makeBlock(1 + (this.x) * 24, 1 + (this.y - enemyBoard.playerGarbage) * 24, xx, yy, this.color, tetrisBoard.canvas);
    ...
    sendLines : function(cleared, tspin) {
        ...
        enemyBoard.sendLines(linesSent); // Send enemy lines

        if(cleared < 1){
            //if incoming lines will set the garbage to higher than 20 blocks
            if(enemyBoard.playerIncoming >= 20 - enemyBoard.playerGarbage) {
                enemyBoard.playerIncoming -= 20 - enemyBoard.playerGarbage;
                enemyBoard.playerGarbage = 20;
            }else{
                enemyBoard.playerGarbage += enemyBoard.playerIncoming;
                enemyBoard.playerIncoming = 0;
            }
        }
        enemyBoard.drawPlayerIncoming();

        console.log("current b2b" + player.b2b);
        ...
        document.getElementById("line").innerHTML += "KO's: " + enemyBoard.playerKo;
